  ...
  function () {
    ...
    _createClass(multiDragBehaviour, [{
      ...
      value: function _addSelection(item) {
        ...
        //and if they're not the same container, clear selections and change container
        ...
        if (typeof this.on_addSelection != 'undefined')
          this.on_addSelection(item);
      ...
      value: function _removeSelection(item) {
        ...
        if (typeof this.on_removeSelection != 'undefined')
          this.on_removeSelection(item);
      ...
      value: function _clearSelections() {
        let items = document.querySelectorAll('[aria-grabbed="true"]');
        if (items.length)
          items.forEach(item => {
            item.setAttribute('aria-grabbed', 'false');
          });

        if (typeof this.on_clearSelections != 'undefined')
          this.on_clearSelections();
      ...
      value: function _dragEnd(e) {
        ...
        if (this.selections.droptarget) {
          ...
          let target = document.querySelector('.drag-over');
          if (target) {
          ...
          for (var len = this.selections.items.length, i = 0; i < len; i++) {
            ...
            if (index+i >= this.selections.droptarget.children.length)
              this.selections.droptarget.appendChild(this.selections.items[i])
            else
              this.selections.droptarget.insertBefore(this.selections.items[i], this.selections.droptarget.children[index+i])
        ...
        if (this.selections.items.length) {
          if (typeof this.on_dragEnd != 'undefined')
            this.on_dragEnd(e, _.cloneDeep(this.selections), index);
...
Index.install = function (Vue) {
  ...
  Vue.directive('mz-drag', {
    bind: function bind(el, binding, vnode) {
      // something logic ...
      el.setAttribute('draggable', !el.classList.contains('ignore')); // enable html5 drag API
